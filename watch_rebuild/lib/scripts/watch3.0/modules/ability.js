...
import { SimpleProjectile, TraceProjectile } from "./projectile";
...
export class Ability {
    ...
    pause() {
        if (this._state != AbilityStateType.Running)
            throw new AbilityProgressError(this._state, AbilityStateType.Pause);
    ...
    main() {
        for (let projectile of this.projectiles)
            if (!projectile.base || !projectile.base.isValid)
     ...
     */
    spawnProjectile(typeId, location, traceMode = TraceModeType.Simple) {
        // Get default location.
        if (!location)
            location = this.projectiles ? this.projectiles[0].base.location : this.user.base.location;
