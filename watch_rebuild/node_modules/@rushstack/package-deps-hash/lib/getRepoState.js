...
async function spawnGitAsync(gitPath, args, currentWorkingDirectory, stdin) {
    ...
    if (stdin) {
         ...
         */
        (0, node_stream_1.pipeline)(stdin, proc.stdin, (err) => { });
    }
    const [status] = await (0, node_events_1.once)(proc, 'close');
    if (status !== 0) {
...
async function hashFilesAsync(rootDirectory, filesToHash, gitPath) {
    const hashPaths = [];
    const input = node_stream_1.Readable.from(isIterable(filesToHash)
        ? (function* () {
