...
function eslintTask(options = {}) {
    return function eslint() {
        ...
        const eslintCmd = (0, just_task_1.resolve)('eslint/bin/eslint.js');
        ...
        const eslintConfigPath = configPath || (0, just_task_1.resolveCwd)('.eslintrc', { extensions: ['.js', '.cjs', '.yaml', '.yml', '.json'] });
        if (eslintCmd && eslintConfigPath && fs.existsSync(eslintConfigPath)) {
            const eslintIgnorePath = ignorePath || (0, just_task_1.resolveCwd)('.eslintignore');
            const eslintArgs = [
                eslintCmd,
                ...(files ? files : ['.']),
            ...
            just_task_1.logger.info((0, utils_1.encodeArgs)(eslintArgs).join(' '));
            return (0, utils_1.spawn)(process.execPath, eslintArgs, { stdio: 'inherit', env });
        }
        else {
