...
function tscTask(options = {}) {
    ...
    return function tsc() {
        ...
        if (isValidProject(options)) {
            ...
            const cmd = (0, utils_1.encodeArgs)([process.execPath, ...args]).join(' ');
            just_task_1.logger.info(`Executing: ${cmd}`);
            return (0, utils_1.exec)(cmd);
        }
...
function tscWatchTask(options = {}) {
    ...
    return function tscWatch() {
        ...
        if (isValidProject(options)) {
            ...
            just_task_1.logger.info((0, utils_1.encodeArgs)(cmd).join(' '));
            return (0, utils_1.spawn)(process.execPath, cmd, { stdio: 'inherit' });
        }
        return Promise.resolve();
    };
}
...
function getProjectOrBuildOptions(options) {
    const tsConfigFile = (0, just_task_1.resolveCwd)('./tsconfig.json') || 'tsconfig.json';
    const result = {};
    if (options.build) {
