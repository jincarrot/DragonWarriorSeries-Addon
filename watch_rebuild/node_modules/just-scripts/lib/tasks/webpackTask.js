...
function webpackTask(options) {
    return async function webpack() {
        const wp = (0, tryRequire_1.tryRequire)('webpack');
        if (!wp) {
            just_task_1.logger.warn('webpack is not installed, this task no effect');
            return;
        }
        just_task_1.logger.info(`Running Webpack`);
        const webpackConfigPath = options && options.config ? (0, just_task_1.resolveCwd)(path.join('.', options.config)) : (0, findWebpackConfig_1.findWebpackConfig)('webpack.config.js');
        ...
        if (webpackConfigPath && fs.existsSync(webpackConfigPath) && webpackConfigPath.endsWith('.ts')) {
            ...
            (0, enableTypeScript_1.enableTypeScript)({ transpileOnly });
        }
        ...
        if (typeof configLoader == 'function') {
            webpackConfigs = configLoader((0, just_task_1.argv)().env, (0, just_task_1.argv)());
        }
        else {
        ...
        webpackConfigs = webpackConfigPromises.map(webpackConfig => (0, webpack_merge_1.merge)(webpackConfig, restConfig));
