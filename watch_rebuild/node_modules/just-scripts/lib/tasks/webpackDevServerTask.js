...
function webpackDevServerTask(options = {}) {
    const configPath = options && options.config
        ? (0, just_task_1.resolveCwd)(path.join('.', options.config))
        : (0, findWebpackConfig_1.findWebpackConfig)('webpack.serve.config.js', 'webpack.config.js');
    // for webpack-cli < 4, use webpack-dev-server directly, for webpack-cli >= 4, use "webpack serve"
    const webpackCliPath = (0, just_task_1.resolve)('webpack-cli/package.json');
    if (!webpackCliPath) {
    ...
    const devServerCmd = useWebpackServe
        ? // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            [(0, just_task_1.resolve)('webpack/bin/webpack.js'), 'serve']
        : // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            [(0, just_task_1.resolve)('webpack-dev-server/bin/webpack-dev-server.js')];
    return function webpackDevServer() {
        ...
        if (configPath && fs.existsSync(configPath)) {
            ...
            options.env = {
                ...options.env,
                ...(configPath.endsWith('.ts') && (0, getTsNodeEnv_1.getTsNodeEnv)(options.tsconfig, options.transpileOnly)),
            };
        }
        if (options.open) {
            args.push('--open');
