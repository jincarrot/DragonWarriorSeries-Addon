...
function copyFilesToDestinationDirectory(sourceFilePaths, destinationDirectory, noSymlinks) {
    return (0, arrayify_1.arrayify)(sourceFilePaths).map(sourceName => ({
        sourceFilePath: (0, path_1.normalize)(sourceName),
        destinationFilePath: (0, path_1.join)(destinationDirectory, (0, path_1.basename)(sourceName)),
...
function copyFileToDestinationDirectoryWithRename(sourceFilePath, destinationName, destinationDirectory, noSymlink) {
    return [{ sourceFilePath, destinationFilePath: (0, path_1.join)(destinationDirectory, destinationName), noSymlink }];
}
...
function copyFilesToDestinationDirectoryWithRename(instrs, destinationDirectory, noSymlinks) {
    return instrs.map(instr => ({
        ...
        destinationFilePath: (0, path_1.join)(destinationDirectory, instr.destinationName),
        noSymlink: noSymlinks,
    }));
}
...
function copyFilesInDirectory(sourceDirectoryPath, outputDirectoryPath, filterFunction, noSymlinks) {
    let files = (0, fs_1.readdirSync)(sourceDirectoryPath);
    if (filterFunction) {
        files = files.filter(filterFunction);
    }
    return files.map(file => ({
        sourceFilePath: (0, path_1.join)(sourceDirectoryPath, file),
        destinationFilePath: (0, path_1.join)(outputDirectoryPath, file),
        noSymlink: noSymlinks,
    }));
}
